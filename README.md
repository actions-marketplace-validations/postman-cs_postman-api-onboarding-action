...
3. **Smoke flow** (`postman-cs/postman-smoke-flow-action`, when `flow-path` or `flow-mode` is set) reshapes the canonical Smoke collection before the built-in test run: from a curated `flow.yaml` manifest when `flow-path` is set, otherwise derived deterministically from the spec under `flow-mode: auto`.
