...
- `working-directory` is forwarded to bootstrap, smoke-flow, and repo-sync so their relative inputs and generated artifacts share one service-local root; Insights has no repository-relative input.
- Existing-repo passthrough inputs to `repo_sync`: `generate-ci-workflow`, `ci-workflow-path`, and `spec-path`. An omitted `generate-ci-workflow` defaults to `true` at the repository root and `false` when `working-directory` is set.
