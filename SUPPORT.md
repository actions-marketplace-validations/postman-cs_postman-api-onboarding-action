...
- Action reference used, such as `postman-cs/postman-api-onboarding-action@v3` or an immutable `v3.x.y` tag.
...
Use `@v3` for the shortest marketplace install path. Use an immutable `@v3.x.y` tag or a commit SHA when reproducibility matters. See [RELEASE_POLICY.md](RELEASE_POLICY.md) for release sequencing and tag policy.
